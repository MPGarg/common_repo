...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, dropout, num_classes=10):
        ...
        self.layer1 = self._make_layer(dropout, block, 64, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(dropout, block, 128, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(dropout, block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(dropout, block, 512, num_blocks[3], stride=2)
        self.linear = nn.Linear(512*block.expansion, num_classes)

    def _make_layer(self, dropout, block, planes, num_blocks, stride):
